-- Primeiro, verifica se a função já existe e a remove se necessário
DROP FUNCTION IF EXISTS public.comparar_participantes_eleitores(UUID, UUID);

-- Cria a função com a implementação correta
...
BEGIN
  ...
  IF pesquisa_uid IS NOT NULL THEN
    v_pesquisa_condition := ' AND pesquisa_uid = ''' || pesquisa_uid::TEXT || '''';
  ELSE
  ...
  v_sql := format(
    'SELECT COUNT(DISTINCT telefone) FROM public.ps_gbp_participantes ' ||
    'WHERE empresa_uid = %L AND telefone IS NOT NULL AND telefone != ''''%s',
    empresa_id,
    v_pesquisa_condition
  );
  
  EXECUTE v_sql INTO v_total_participantes;
  ...
  EXECUTE v_sql INTO v_correspondencias;
...
GRANT EXECUTE ON FUNCTION public.comparar_participantes_eleitores(UUID, UUID) TO anon, authenticated, service_role;
